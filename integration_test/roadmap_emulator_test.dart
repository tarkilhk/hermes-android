...
void main() {
  ...
  testWidgets(
    'recovered prompts background tasks and answer versions keep server ownership',
    (tester) async {
      harness.fixture.resumeSnapshots['personal/chat-0'] = {
        'running': false,
        'pending_sensitive': {
          'type': 'vault.code.request',
          'payload': {
            'request_id': 'roadmap-code',
            'site': 'Fixture Console',
            'hint': 'Enter the expiring fixture code',
          },
        },
        'side_tasks': const {'retention': 'live_session', 'tasks': <Object>[]},
      };
      harness.fixture.resumeSnapshots['personal/chat-1'] = {
        'running': false,
        ...
        'side_tasks': {
          ...
          'tasks': [
            {
              'task_id': 'roadmap-background',
              'kind': 'background',
              'prompt': 'Check the isolated emulator state',
      ...
      harness.fixture.versionHistories['personal/version-older'] = const [
        {
          'id': 901,
          'role': 'user',
          'content': 'personal message 619',
          'display_content': 'personal message 619',
        },
        {
          'id': 902,
          'role': 'assistant',
          'content': 'Authoritative earlier fixture answer',
          'display_content': 'Authoritative earlier fixture answer',
        },
      ];
      final versions = {
        'source': {'session_id': 'chat-1', 'answer_row_id': 620},
        'versions': [
          {'session_id': 'version-older', 'answer_row_id': 902, 'position': 0},
          {'session_id': 'chat-1', 'answer_row_id': 620, 'position': 1},
        ],
      };
      harness.fixture.answerVersionResponses['personal/chat-1/620'] = versions;
      harness.fixture.answerVersionResponses['personal/version-older/902'] =
          versions;
      harness.fixture.liveSessions['personal'] = [
        {
          'id': 'runtime-chat-1',
      ...
      await _pumpUntil(
        tester,
        () => find
            .byKey(const Key('sensitive-prompt-field'))
            .evaluate()
            .isNotEmpty,
      );
      final sensitiveChat = harness.controller.current!.chat!;
      expect(sensitiveChat.key.workspace.profileName, 'personal');
      expect(find.text('Enter code for Fixture Console'), findsOneWidget);
      expect(find.text('Enter the expiring fixture code'), findsOneWidget);

      harness.fixture.expireVaultCode(
        'personal',
        sensitiveChat.runtimeId,
        'roadmap-code',
      );
      await tester.pump();
      ...
      await _pumpUntil(
        tester,
        () => find.text('Background task running').evaluate().isNotEmpty,
      );
      final sourceChat = harness.controller.current!.chat!;
      expect(sourceChat.key.sessionId, 'chat-1');
      expect(sourceChat.busy, isFalse);
      expect(find.text('Check the isolated emulator state'), findsOneWidget);
      expect(find.text('Waiting for the Hermes host.'), findsOneWidget);

      await _navigate(tester, AppDestination.activity);
      await _pumpUntil(
        tester,
        () => find
            .textContaining('personal · Background work running')
            .evaluate()
            .isNotEmpty,
      );
      expect(find.text('personal chat 1'), findsOneWidget);
      await tester.tap(find.byKey(const ValueKey('activity-personal-chat-1')));
      await _pumpUntil(
        tester,
        () => find
            .byKey(const ValueKey('answer-actions-620'))
            .evaluate()
            .isNotEmpty,
      );

      final sourceActions = find.byKey(const ValueKey('answer-actions-620'));
      await tester.ensureVisible(sourceActions);
      await _pumpUntil(
        tester,
        () => find
            .descendant(of: sourceActions, matching: find.text('2 / 2'))
            .evaluate()
            .isNotEmpty,
      );
      await tester.tap(
      ...
      await _pumpUntil(
        tester,
        () => find
            .text('Authoritative earlier fixture answer')
            .evaluate()
            .isNotEmpty,
      );
      expect(harness.controller.current?.chat?.key.sessionId, 'version-older');
      expect(find.text('Authoritative earlier fixture answer'), findsOneWidget);

      final earlierActions = find.byKey(const ValueKey('answer-actions-902'));
      await _pumpUntil(
        tester,
        () => find
            .descendant(of: earlierActions, matching: find.text('1 / 2'))
            .evaluate()
            .isNotEmpty,
      );
      await tester.tap(
      ...
      await _pumpUntil(
        tester,
        () => find.text('personal message 620').evaluate().isNotEmpty,
      );
      expect(harness.controller.current?.chat?.key.sessionId, 'chat-1');
      expect(find.text('personal message 620'), findsOneWidget);

      final resumeCalls = harness.fixture.calls.where(
        (call) => call.$2 == 'session.resume',
      );
      expect(
        resumeCalls.where((call) => call.$3['session_id'] == 'chat-0'),
        isNotEmpty,
      );
      expect(
        resumeCalls.where((call) => call.$3['session_id'] == 'version-older'),
        isNotEmpty,
      );
      expect(
        resumeCalls.every(
          (call) => call.$1 == 'personal' && call.$3['profile'] == 'personal',
        ),
        isTrue,
      );
      expect(
        harness.fixture.calls
            .where((call) => call.$2 == 'session.answer_versions')
            .every(
              (call) =>
                  call.$1 == 'personal' && call.$3['profile'] == 'personal',
            ),
        isTrue,
      );
      expect(tester.takeException(), isNull);

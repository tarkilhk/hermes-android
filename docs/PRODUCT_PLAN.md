...
Next verified release, 2026-09-12: Personal 2.27.1 / 21792 fixes notification startup recovery and gives each original chat a deterministic alert ID. It includes the 2.27.0 attachment queues and preceding Find, read-state and background-task improvements. All 24 focused notification checks and 1,216 full-suite tests pass, with four opt-in skips. Analyzer clean; the signed APK passed native compilation and certificate/package checks. Installation is deferred while the owner is away from home. The next client correction is duplicate notification tap navigation. Firebase configuration and the trusted backend sender remain necessary for delivery while the app is absent; see [background delivery](BACKGROUND_NOTIFICATIONS.md). The delivery sequence retains all other phone/live QA and backend-contract dependencies. All selected work and exclusions remain intact.

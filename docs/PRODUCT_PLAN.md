...
Latest phone delivery, 2026-09-12: Hermes Personal 2.15.0 / 21642 is installed and launched. It includes subagent supervision, goals, session background work, diagnostics, installed version display, profile/SOUL editing and one-host backend updates. The D15 context reopen fix and recent deletion fixes are preserved. The installed 2.15.0 snapshot passed 1,124 tests with four opt-in skips and a clean analyzer. Custom access-proxy headers are included. Server usage is implemented in source 2.13.0 (1,103 tests, four opt-in skips); goal criteria editing is implemented in 2.14.0 (1,109 tests, four skips, clean analyzer). Selected-host updates are implemented in source 2.16.0 (1,132 tests, four skips, clean analyzer). A verified remote TUI restart contract, remaining selected features and live QA stay tracked below.

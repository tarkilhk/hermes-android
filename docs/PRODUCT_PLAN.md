...
The original source check found active-turn paths for `/steer`, `/btw`, `/bg` and `/background`. Typing a slash draft changes the busy composer button from Stop to Send. D04 subsequently added the dedicated current-session `/yolo` handler. D11 added correlated `/btw` cards; `/bg` still needs the same identifiable pending/result flow. Task recovery after reconnect remains a separate missing server contract. The [contract notes](research/FEATURE_PLAN_CONTRACT_NOTES_2026-09-11.md) preserve the original findings.
...
Next verified release, 2026-09-12: Personal 2.24.1 / 21742 fixes large-chat Find and known compressed-history segment handling. Full suite: 1,176 passed, four opt-in skips; analyzer clean; an additional narrow error/keyboard check passed. The signed APK passed certificate/package checks. Installation is waiting for a new wireless-debugging endpoint because the phone refused 10.30.1.2:42969. D11 background cards, C07 read lifecycle, Find result navigation and attachment queues are the next selected deliveries.

...
| W06 Unblock and supervise work | R04, R05, R07, R08, R09, R10, R11, R12, R13, R14, R15, R18 | Answer requests and inspect/control active agent work through supported server contracts. | In progress: D06/D07 and D23-D25 initial controls implemented; criteria editing and live QA remain |
...
| W08 Connections and operations | B01, B02, B04, B10, B11 all proposed, B14, B15, S01, S02 | Current-looking password setup and repair, understandable health/usage, scoped profile/admin actions and backend updates. | Initial diagnostics in progress; profile editing/usage/update work remains |
| W09 App navigation and preferences | S08, S10, left drawer | Consistent navigation, version/build visibility and readable preferences. Server session restoration belongs to W01. | Navigation/preferences implemented; version visibility in progress |
...
Latest delivery, 2026-09-12: Personal 2.8.0 / 21572 adds server goal details and controls, preserving subagent supervision, Camera, the D15 reopen fix and recent chat-deletion fixes. It is installed and launched on the owner's phone; 1,042 tests passed, four opt-in skips, analyzer clean. See [session controls](SESSION_CONTROLS.md) for the initial scope and outstanding criteria editing/live QA. Source milestones 2.9.0 (background controls) and 2.10.0 (diagnostics/version visibility) are implemented; 2.10.0 passed 1,072 tests with four opt-in skips and a clean analyzer. Phone deployment is pending because wireless debugging became unavailable. D29 profile editing is implemented in 2.11.0 (1,080 tests, four skips, clean analyzer); D30 one-host updates are implemented in 2.12.0 (1,097 tests, four skips, clean analyzer); multi-host updates and a verified TUI restart contract remain; all other selected milestones and exclusions remain in the plan.

...
Next verified release, 2026-09-12: Personal 2.27.0 / 21782 adds attachment-bearing queues, filename review, preserved composer work and durable file handling after failures or uncertain sends. It includes View in chat from Find, the C07 read lifecycle, background-task cards and paged Find/compressed-history fixes. Full suite: 1,209 passed, four opt-in skips; analyzer clean; all 108 focused queue/draft/send checks passed. Independent review found no remaining issue, and the signed APK passed native compilation and certificate/package checks. Installation is deferred while the owner is away from home. The current continuation queue is implemented; the delivery sequence records remaining phone/live QA, Firebase setup and backend-contract dependencies. All selected work and exclusions remain intact.

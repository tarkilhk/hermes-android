...
| W08 Connections and operations | B01, B02, B04, B10, B11 all proposed, B14, B15, S01, S02 | Current-looking password setup and repair, understandable health/usage, scoped profile/admin actions and backend updates. | Diagnostics, profile editing, usage and one-host updates implemented; custom access headers implemented; multi-host work remains |
...
Latest phone delivery, 2026-09-12: Hermes Personal 2.14.0 / 21632 is installed and launched. It includes subagent supervision, goals, session background work, diagnostics, installed version display, profile/SOUL editing and one-host backend updates. The D15 context reopen fix and recent deletion fixes are preserved. The installed 2.14.0 snapshot passed 1,109 tests with four opt-in skips and a clean analyzer. Server usage is implemented in source 2.13.0 (1,103 tests, four opt-in skips); goal criteria editing is implemented in 2.14.0 (1,109 tests, four skips, clean analyzer). Multi-host updates, a verified remote TUI restart contract, remaining selected features and live QA stay tracked below.

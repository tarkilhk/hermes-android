...
Next verified release, 2026-09-12: Personal 2.26.0 / 21772 adds View in chat from Find results, including older messages and expanded tool matches with Back to latest. It includes the C07 read lifecycle, background-task cards and paged Find/compressed-history fixes. Full suite: 1,191 passed, four opt-in skips; analyzer clean; all 22 focused Find/reading checks passed. The signed APK passed native compilation and certificate/package checks. Installation is deferred while the owner is away from home. Attachment queues are the next selected delivery; all other planned work and documented dependencies remain intact.

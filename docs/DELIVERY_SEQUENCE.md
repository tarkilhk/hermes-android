...
| D05 | Implemented; active-row live QA pending | Per-profile `session.active_list` discovery, original session ownership and partial-profile errors | Live 2.31.1 opened Activity without error when the server returned no ongoing sessions | Verify active and input-required rows, filters and cross-profile ownership with live work |
...
| D08 | Local test alert verified; chat routing QA pending | Independent completion/attention switches, optional chat titles, permission/test alert and existing notification routing; 2.27.1 adds startup retry and stable chat alert IDs | Samsung 2.31.1 posted the built-in test alert with Android permission granted; tapping it returned to Hermes | Verify a real chat-specific completion/input alert; locked/terminated-app push remains D26/D27 |
...
| D21 | Implemented; Samsung text restart QA passed | Connection/profile/new-or-existing chat choice, atomic draft staging, private durable pending intake and exact asynchronous acknowledgement/discard | Samsung 2.31.1 recovered exact incoming text after force-stop; Home Review reopened it, and Discard remained effective after another restart | File/capture recovery and third-party share-sheet selection remain. An interruption between draft save and intake acknowledgement can reoffer a share |
| D22 | Implemented; Samsung cancellation QA passed | Camera/Photos/Files feed existing draft preparation; camera returns to review with a verified originating chat or explicit destination choice. See [capture notes](SHARING_AND_CAPTURE.md) | Samsung 2.31.1 cancelled Photos, Files and Camera back to an empty composer; a second Camera launch succeeded | Successful capture, media intake and capture process-recreation QA remain |

...
| D15 | Reopen bug reported; fix in progress | Border-integrated server context fuse and endpoint dot; live usage and unknown/estimated states | Model/widget tests, edge geometry and stale-response/profile isolation checks | Owner reports on 2026-09-12 that existing chats open without loading their context fullness. Fix authoritative usage loading on reopen; do not require a new prompt or estimate locally. |
...
| D21 | Review flow implemented; native intake follow-up remains | Connection/profile/new-or-existing chat choice, atomic staging into existing drafts, exact pending-share acknowledgement and retry/discard | Merge/failure/routing tests | Pre-destination pending metadata remains in memory; native intake interruption/error handling remains to complete |
| D22 | Photos implemented; Camera planned | Photos and Files choices reuse the picker and attachment preparation | Existing attachment checks | Camera needs a small Android acquisition bridge |
| D23-D30 | Planned | Not started | None | Check relevant deployed contracts at slice start |

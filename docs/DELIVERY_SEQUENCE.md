...
| D26-D27 / M01, R16, R17, S12, M09 | Configure the Firebase project/app and trusted backend sender, then implement authenticated registration and delivery through the existing notification routing. [Setup and verified dependencies](BACKGROUND_NOTIFICATIONS.md) distinguish agent notices from completion/input events. Client initialization retry and stable notification IDs are verified in 2.27.1. Duplicate tap navigation is the next client correction. Locked/background/terminated-app tests remain part of completion. |
...
| D08 | Implemented; live QA pending | Independent completion/attention switches, optional chat titles, permission/test alert and existing notification routing; 2.27.1 adds startup retry and stable chat alert IDs | 24 focused checks; 1,216 full-suite passes/four skips; analyzer clean; signed Personal 21792 verified | Phone permission/tap checks remain; duplicate tap navigation and locked/terminated-app push remain D26/D27 |

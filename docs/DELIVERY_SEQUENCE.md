...
| D26-D27 / M01, R16, R17, S12, M09 | Configure the Firebase project/app and trusted backend sender, then implement authenticated registration and delivery through the existing notification routing. [Setup and verified dependencies](BACKGROUND_NOTIFICATIONS.md) distinguish agent notices from completion/input events. Client initialization retry and stable notification IDs are verified in 2.27.1. Duplicate tap navigation is verified in 2.27.2; phone checks remain. Locked/background/terminated-app tests remain part of completion. |
...
| D26-D27 | Client routing corrected; Firebase setup pending | Reuse local notifications, retryable initialization, stable IDs and authenticated tap routing with duplicate/obsolete-open handling | 2.27.2: 33 focused checks and 1,224 full-suite tests passed/four skips; analyzer clean; signed Personal 21802 verified. See [delivery dependencies](BACKGROUND_NOTIFICATIONS.md) | Firebase project/app configuration and trusted backend sender integration remain necessary; no Firebase SDK added yet. Locked/background/terminated-app phone QA remains |

...
| D24 | Initial slice implemented; live QA pending | Server goal state, criteria/verification details and acknowledged Pause/Resume/Unwait/Clear. See [session controls](SESSION_CONTROLS.md) | 1,042 tests passed, four opt-in skips; clean analyzer; signed Personal 2.8.0 / 21572 installed and launched | Criteria editing remains a later portion of R13; live-server controls QA remains |
| D25 | Implemented; phone deployment pending | Session heartbeat/loop state, supported controls and targeted process Stop/Dismiss | 1,057 tests passed, four opt-in skips; clean analyzer; signed Personal 2.9.0 / 21582 built and verified | Wireless endpoint refused connection after build; phone remains on 2.8.0. No full process-output RPC; Dismiss is transient client view state; live controls QA remains |
...
| D28 | Initial diagnostics implemented; live QA pending | Manual dashboard/authentication and provider checks, with existing Manage connections repair route | 1,072 tests passed, four opt-in skips; clean analyzer | Usage and remaining access/repair gaps stay tracked |
| D29 | Initial editor implemented; live QA pending | Selected-profile description/SOUL editor with acknowledged and partial-save handling | 1,080 tests passed, four opt-in skips; clean analyzer | Phone deployment awaits wireless debugging. No global profile activation or broad admin |
| D30 | Versions and one-host updates implemented | Installed Android identity; fresh update eligibility, confirmation, progress, correlated receipt outcomes | 2.12.0: 1,097 tests passed, four opt-in skips; clean analyzer | TUI restart needs a verified remote endpoint; multi-host operations and live update QA remain |

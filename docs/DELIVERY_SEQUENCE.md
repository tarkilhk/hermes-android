...
| D23 | Implemented; release validation in progress | Scoped subagent roster, live details and acknowledged Steer/Interrupt. See [subagent notes](SUBAGENT_SUPERVISION.md) | Installed backend contract inspected; 1,029 tests passed, four opt-in skips | Active-only roster and transient tails; live-server/phone controls QA remains |
| D24 | In progress | Structured server goal state, details and supported controls | Installed `session.control` contract inspected | Criteria editing remains a later portion of R13 |
| D25-D30 | Planned | Not started | None | Check relevant deployed contracts at slice start |

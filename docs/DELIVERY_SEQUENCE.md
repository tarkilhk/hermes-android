...
| D30 | Version visibility implemented; update operations in progress | Installed Android identity and manual backend update check | Included in 2.10.0 checks | TUI backend restart has no verified remote endpoint; one-host update, then multi-host outcomes remain |

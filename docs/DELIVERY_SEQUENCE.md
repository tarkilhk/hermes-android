...
| D09 | Samsung text/file queues and one-shot Steer passed | Desktop-style text/file queues with filename review and removal; next-draft attachment preparation survives active responses and picker reconnection | Samsung 2.31.6 verified ordered automatic drain and a separate reply, plus acknowledged Steer and cleared draft. See the [QA sweep](QA_SWEEP_2026-09-13.md) | Lost-ack attachment interruption remains unverified. QA018 file-content expansion remains a backend dependency |
| D10 | Ordinary text actions passed; synchronized versions deferred | Edit/resend, Regenerate, Branch, Fork and existing parent-chat navigation. Unsupported version-group UI and requests are removed. See [answer versions](ANSWER_VERSIONS.md) | Emulator controls passed with exact scoped requests, retained unrelated draft, parent navigation and one-shot Fork. Samsung plain-text Branch, Regenerate and Edit/resend passed after reopen; see the [QA sweep](QA_SWEEP_2026-09-13.md) | 2.31.8 corrects QA019 to current Desktop same-chat regeneration; its attachment-bearing live retest is pending. Explicit synchronized versions are not verified on unmodified Hermes |

...
`session.active_list` returns a process-global `sessions` snapshot with runtime `id`, durable `session_key`, `status` and optional `last_active`. It does not identify the owning profile. Desktop treats `working` and `waiting` as active and reconciles absence from a successful snapshot. Android must read this snapshot once, then join each durable ID against profile-scoped saved metadata. It may also use an exact locally known runtime and durable ID pair. Unknown or ambiguous owners stay hidden because opening a row under the wrong profile can adopt or mutate another profile's session. `starting` is also presented as running.

Activity enumeration must not resume sessions. Resume can attach, create or adopt a runtime, so call it only after the client has verified the selected session's owner. Profile-scoped exact search results establish saved ownership; liveness still comes from the global runtime snapshot. If any profile cannot be checked, saved metadata cannot prove uniqueness across profiles. Hide those unverified rows and report that coverage is incomplete.

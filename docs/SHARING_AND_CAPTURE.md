...
## Camera recovery in 2.31.4 and 2.31.5

A locally created, unsent chat can expire while Camera is open. Same-process recovery keeps the existing draft and chat settings, and camera return joins any reconnect already replacing that empty session. Samsung 2.31.4 passed capture, return, draft preservation and a subsequent text-only send. Saved-chat capture also passed after verified background process removal and relaunch.

When the process has been removed and the exact original session cannot be resumed, 2.31.5 offers **New chat with recovered draft** if that connection/profile/session has a local draft. The review shows its text and attachment/queue counts. Choosing recovery moves that saved draft into a fresh chat in the original profile. It preserves delivery uncertainty, pauses queued messages, and resets attachment references for the new session. Missing cached files remain visible for removal or reattachment. Send remains a separate action.

The saved draft moves in one storage write before the captured content is staged. If staging fails, the recovered draft remains saved and retries reuse the new chat. This uses the existing draft store and a temporary in-memory target reference. It does not persist conversation state or infer that the missing server session was never used. Ordinary cold startup also exposes Saved drafts in Chats as of 2.31.7, using the same storage and recovery path.

The new process-restart recovery passed on Samsung 2.31.5 / 21902 after verified background process removal: review showed the original text, recovery staged it with the photo, and the recovered chat completed a text-only prompt after the photo was removed unsent. All 1,312 unit/widget tests pass, with four opt-in skips, and analysis is clean. The current cross-feature evidence is in the [QA sweep](QA_SWEEP_2026-09-13.md).

## Ordinary draft recovery and native intake in 2.31.7

Chats shows Saved drafts for its selected connection and profile when a locally saved draft has no loaded chat row. Opening a draft first tries the original server session. Only a verified missing-session reply permits recovery into a fresh chat. Text, cached files, paused queues and uncertainty are preserved; Send remains explicit. Samsung recovered the original failed-case draft and repeated recovery after another verified process removal.

Samsung My Files shared both a text file and PNG through the Hermes Personal share target, review and the selected existing chat. Photos selection through Gallery also staged the exact PNG. Those fixtures were removed unsent. Picker-return reconnection and automatic attachment queue drain passed in 2.31.6; backend file-content expansion remains QA-018.
